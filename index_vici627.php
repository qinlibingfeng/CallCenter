<?php 
	...
	}else if($action == "SynCallRecordingLogs"){//同步录音路径
	...
	}else if($action == "SynMissedCalls"){ //同步漏接来电
...
function syn_users($server_ip, $server_dbname ,$server_login, $server_pass, $user_name_enc, $user_pass_enc, $user_fullname_enc, $user_level_enc, $user_active_enc, $user_group_enc){
	...
	{
			$stmt = 'insert into agents (code, name, passwd, role_id, department_id) values ("'.$user_name.'", "'.$user_fullname.'", "'.$user_pass.'" , "'.$user_role_id.'", "'.$user_department_id.'");';
