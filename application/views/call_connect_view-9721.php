						...
						open: function( event, ui ) {			
							...
							$.post("<?php echo site_url("dictionary/ajaxGetKeyValue")?>",req,function(ret){
								$.each(ret,function(index,row){
									if(row.name_text != '未填写')
										$('#orderformType').append("<option value='"+row.name_text+"'>"+row.name_text+"</option>");
										else
										$('#orderformType').append("<option selected='selected' value='"+row.name_text+"'>"+row.name_text+"</option>");
								});
							});
							
							//获取反应问题值
							$('#orderformContent').attr('value',$("#client_note").val());	
...
<div id="createOrderForm-dialog"  style="display:none">
	反应问题:<textarea id="orderformContent"></textarea><br />	
    订单类型:<select id="orderformType"></select>&nbsp;&nbsp;&nbsp; 接收人:<select id="Orderform-reciever"></select> <br>
	截止时间:<input id='Orderform-ymd' type="text" style="width:90px" value="<?php echo $yuyue['ymh']?>">&nbsp;<?php echo form_dropdown('s_hour',$yuyue['hourOptions'],$yuyue['hourDef'],'id="Orderform-hour"')?>&nbsp;<?php echo form_dropdown('s_min',$yuyue['minOptions'],$yuyue['minDef'],'id="Orderform-min"')?>
</div>
<div class="page_main page_tops">
	<div class="page_nav">
     ...
     <div class="func-panel">
        ...
        <div class='content'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;姓名: <span class="person-info"><?php echo isset($clientItem[0])?$clientItem[0]['client_name']:''; ?></span>&nbsp;所属坐席：<span class="person-info"><?php echo isset($clientItem[0]['client_agent'])?$clientItem[0]['client_agent']:''; ?></span> &nbsp;电话：<span class="person-info"><?php echo isset($clientItem[0])?$clientItem[0]['client_cell_phone']:''; ?> </span> &nbsp; 地址： <span class="person-info"><?php echo isset($clientItem[0]['client_address'])?$clientItem[0]['client_address']:'';?> &nbsp; </span> 号码归属地：<span class="person-info"><?php echo $gsd;?></span></div>
